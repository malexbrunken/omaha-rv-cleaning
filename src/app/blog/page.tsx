...
export default function BlogPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <h1 className="text-4xl font-extrabold text-espresso mb-4">RV Care Guides</h1>
      ...
      <div className="space-y-6">
        {posts.map((p) => (
          <article key={p.slug} className="rounded-2xl border-2 border-line p-6">
            ...
            <h2 className="text-2xl font-bold text-espresso mb-2">
              <Link href={`/blog/${p.slug}`} className="hover:text-copper">{p.title}</Link>
            </h2>
            <p className="text-lg text-mist mb-3">{p.excerpt}</p>
            <Link href={`/blog/${p.slug}`} className="text-copper font-semibold underline">Read the guide →</Link>
          </article>
        ))}
      </div>
    </div>
