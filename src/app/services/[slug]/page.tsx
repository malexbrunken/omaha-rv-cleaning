  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <nav aria-label="Breadcrumb" className="text-muted mb-4">
        ...
        <Link href="/services" className="text-accent hover:underline">Services</Link> › {s.name}
      </nav>

      <h1 className="text-4xl font-extrabold text-navy mb-5">{s.h1}</h1>
      ...
      <ul className="space-y-2.5 mb-10 rounded-2xl bg-cream border border-siteborder p-6">
        {s.includes.map((i) => (
          <li key={i} className="flex gap-3 text-lg"><span className="text-accent font-bold">✓</span> {i}</li>
        ))}
      </ul>
      ...
      <h2 className="text-2xl font-bold text-navy mt-10 mb-4">Questions we hear a lot</h2>
      {s.faq.map((f) => (
      ...
      <h2 className="text-2xl font-bold text-navy mt-12 mb-4">Related services</h2>
      <div className="grid gap-4 sm:grid-cols-3">
        {others.map((o) => (
          <Link key={o.slug} href={`/services/${o.slug}`} className="rounded-xl border-2 border-siteborder p-5 hover:border-accent">
      ...
      <p className="mt-8">
        ...
        <Link href="/service-areas" className="text-accent underline font-semibold">service-area pages</Link> or view{" "}
        <Link href="/pricing" className="text-accent underline font-semibold">transparent pricing</Link>.
      </p>
    </div>
  );
}
