...
export default function ServiceAreasPage() {
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <h1 className="text-4xl font-extrabold text-navy mb-4">Where We Clean</h1>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {towns.map((t) => (
          <Link key={t.slug} href={`/service-areas/${t.slug}`} className="rounded-xl border-2 border-siteborder p-6 hover:border-accent">
            <p className="font-bold text-navy text-lg">{t.name}, NE</p>
            <p className="text-muted text-sm">{t.county}</p>
          </Link>
        ))}
      </div>
    </div>
